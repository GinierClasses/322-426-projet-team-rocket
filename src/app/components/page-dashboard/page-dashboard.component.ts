...
export class PageDashboardComponent implements OnInit {
  ...
  informatiqueAverage = 0;
  cultureGAverage = 0;
  baseElargieAverage = 0;
  cfcAverage = 0;
  ...
  calculateAverages(modules: ModuleModel[]): void {
    ...
    const allAverages = [
      this.informatiqueAverage,
      this.cultureGAverage,
      this.baseElargieAverage,
    ];
    this.cfcAverage =
      Math.round(
        (allAverages.reduce((sum, average) => sum + average, 0) /
          allAverages.length) *
          10
      ) / 10;
  }
